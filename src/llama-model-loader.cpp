...
const struct ggml_tensor * llama_model_loader::check_tensor_dims(const std::string &          name,
...

    {
        ...
        // Special handling for TinyMoE gate weights: allow transposed shape for backward compatibility
        // Expected: (n_embd, n_expert), but may get (n_expert, n_embd, 1, 1) from old conversions
        bool is_gate_weight = name.find("ffn_gate_inp.weight") != std::string::npos;
        if (is_gate_weight && ne.size() == 2) {
            int64_t expected_ne0 = ne[0];
            int64_t expected_ne1 = ne[1];
            int64_t actual_ne0   = cur->ne[0];
            int64_t actual_ne1   = cur->ne[1];

            // Check if dimensions are transposed and extra dims are 1
            if (actual_ne0 == expected_ne1 && actual_ne1 == expected_ne0) {
                // Check that extra dimensions are all 1
                bool extra_dims_ok = true;
                ...
                if (extra_dims_ok) {
                    // Accept transposed shape for backward compatibility
                    // Note: New models should be converted with the updated conversion script
                    LLAMA_LOG_WARN(
                        "%s: tensor '%s' has transposed shape (got %s, expected %s). This is accepted for backward "
                        "compatibility, but please reconvert with the updated conversion script for optimal "
                        "performance.\n",
                        __func__, name.c_str(), llama_format_tensor_shape(cur).c_str(),
                        llama_format_tensor_shape(ne).c_str());
                    is_ok = true;  // Accept this shape
                } else {
                    is_ok = false;
                }
            } else {
                // Not transposed, check normally
            ...
            // Normal dimension check for non-gate weights
            for (size_t i = 0; i < GGML_MAX_DIMS; ++i) {
                if ((i < ne.size() && ne[i] != cur->ne[i]) || (i >= ne.size() && cur->ne[i] != 1)) {
                    is_ok = false;
                    break;
